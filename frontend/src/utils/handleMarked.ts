    ...
    const marked = new Marked(
        markedHighlight({
            ...
            highlight: (code, lang) => {
                ...
                let codeString = hljs.highlight(code, {language}).value;

                let docBody = new DOMParser().parseFromString(codeString, 'text/html').body;
                let newElement = Object.assign(document.createElement("p"), {
                    textContent: code,
...
export const generateCodeBlockCopyButton = (className: string) => {
    setTimeout(()=>{
        let codeBlocks = document.querySelectorAll(`.${className} pre`);
        for( let codeBlock of codeBlocks ) {
            let sourceEle = codeBlock.querySelector('code .source')
            if (sourceEle) {
                // 动态创建CopyButton元素，挂载到pre内
                let source = sourceEle.textContent;
                let mountEle = Object.assign(document.createElement("div"), {
...
export const getTocRender = (tocObj: TocHandler) => {
    ...
    renderer.heading = function({ tokens, depth }){
        ...
        let anchor = tocObj.add(text, depth);
