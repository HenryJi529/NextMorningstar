...
const updateUserLockStatus = async () => {
    const response: R<object> = (
        await axios.get(`${API_USER_MANAGE_LOCK}?id=${id}&isLocked=${isLocked.value}`)
    ).data;
...
const sendRecoverEmailToUser = async () => {
    const response: R<object> = (
        await axios.get(`${API_USER_MANAGE_RECOVER}?id=${id}&email=${email.value}`)
    ).data;
    if (response.code !== ResponseCode.SUCCESS) {
        errorMessage.value = response.msg;
    } else {
...
const updateUserRoleSetting = async () => {
    const response: R<object> = (
        await axios.put(API_USER_MANAGE_ROLE, {
            userId: id,
...
const loadPrivateInfo = async () => {
    const response: R<UserPrivateInfo> = (
        await axios.get(API_USER_MANAGE_INFO_ONE.replace('{id}', id))
    ).data;
    if (response.code !== ResponseCode.SUCCESS) {
    ...
    currRoles.value = response.data.permissions
        .filter(permission => {
...
<template>
    <div class="flex justify-center items-center mt-10">
        <div v-if="itemNotExist">用户不存在</div>
        <div v-else class="w-[40rem] p-4">
            ...
            <div class="mx-4">
                ...
                <div class="w-full flex items-center">
                    <div
                        class="admin-manage-input flex items-center flex-wrap justify-start min-h-[54px]">
                        <template v-for="(currRole, ind) in currRoles" :key="currRole">
                            <div
                ...
                <div class="flex items-center flex-wrap pl-2 pr-8 pt-1 mb-4">
                    <template
                        v-for="optionalRole in allRoles.filter(role => !currRoles.includes(role))"
                        :key="optionalRole">
                        <div
