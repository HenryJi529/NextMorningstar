...
const updateUserLockStatus = async () => {
    const response: R<object> = (await axios.get(`${API_USER_MANAGE_LOCK}?id=${id}&isLocked=${isLocked.value}`, {
        headers: {
...
const sendRecoverEmailToUser = async () => {
    const response: R<object> = (await axios.get(`${API_USER_MANAGE_RECOVER}?id=${id}&email=${email.value}`, {
        headers: {
            authorization: localStorage.getItem(TOKEN),
        }
    })).data;
...
const loadAllRoles = async () => {
    const response: R<string[]> = (await axios.get(API_USER_MANAGE_ROLE, {
        headers: {
            Authorization: localStorage.getItem(TOKEN)
        }
    })).data;
        ...
        <div v-else class="w-[40rem] p-4">
            ...
            <div>
                ...
                <div class="flex justify-center items-center space-x-4 mb-4">
                    <div><font-awesome-icon :icon="['fas', 'lock-open']" /></div>
                    <input type="checkbox" class="toggle" v-model="isLocked" />
                    <div><font-awesome-icon :icon="['fas', 'lock']" /></div>
                </div>
            ...
            <div class="mx-4">
                ...
                <div class="flex-1 input input-bordered flex items-center space-x-2 mb-4">
                    ...
                    <input type="text" placeholder="用户邮箱" v-model="email" class="w-full">
                </div>
            ...
            <div class="mx-4">
                ...
                <div class="flex items-center flex-wrap pl-2 pr-8 pt-1 mb-4">
                    <template v-for="optionalRole in allRoles.filter(role => !currRoles.includes(role))">
                        <div @click="currRoles.push(optionalRole)"
