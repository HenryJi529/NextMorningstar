...
import { API_LOVE_RANDOM, API_LOVE_SELECTED } from '@/constants/api';
...
function createTouchEnter() {
    const touchEnterEvent = new Event('touchEnter');
    const elementContains = (parent: Element, child: Element) =>
...
function getAverageColor(fills: string[]) {
    const r = Math.floor(
        fills
            ...
            .map(c => parseInt(c, 16))
            .reduce((accumulator, currentValue) => accumulator + currentValue, 0) / fills.length
    );
    const g = Math.floor(
        fills
...
function getCurrentFill(currentCx: number, currentCy: number, currentLevel: number): string {
    if (currentLevel === 1) {
        const w = (currentCx - minBlockWidth / 2) / minBlockWidth;
        const h = (currentCy - minBlockHeight / 2) / minBlockHeight;
        return lovePhotoData.fills[h][w];
    } else {
        // 获取左上角的填充
        const cxA = currentCx - minBlockWidth / 2;
        const cyA = currentCy - minBlockHeight / 2;
        const fillA = getCurrentFill(cxA, cyA, 1);
        // 获取左下角的填充
        const cxB = currentCx - minBlockWidth / 2;
        const cyB = currentCy + minBlockHeight / 2;
        const fillB = getCurrentFill(cxB, cyB, 1);
        // 获取右上角的填充
        const cxC = currentCx + minBlockWidth / 2;
        const cyC = currentCy - minBlockHeight / 2;
        const fillC = getCurrentFill(cxC, cyC, 1);
        // 获取右下角的填充
        const cxD = currentCx + minBlockWidth / 2;
        const cyD = currentCy + minBlockHeight / 2;
        const fillD = getCurrentFill(cxD, cyD, 1);
...
function createEllipse(currentCx: number, currentCy: number, currentLevel: number) {
    ...
    ellipse.addEventListener(
        ...
        e => {
            if (currentLevel > 1) {
                const ele = e.target as SVGEllipseElement;
                ele.classList.add('animate__fadeOut');
                ele.addEventListener('animationend', function () {
                    svg.value.removeChild(ele);
                });

                const nextCxList = getNextCxList(currentCx, currentLevel);
                const nextCyList = getNextCyList(currentCy, currentLevel);
                const nextLevel = currentLevel - 1;

                for (let i = 0; i < 4; i++) {
                    const child = createEllipse(nextCxList[i], nextCyList[i], nextLevel);
...
function autoFinish() {
    const autoCheck = window.setInterval(() => {
...
<template>
    <div class="mx-auto flex flex-col justify-center items-center w-[24rem]">
        <img
            v-if="isDone"
            :alt="lovePhotoData.photoName"
            :src="lovePhotoData.base64String"
