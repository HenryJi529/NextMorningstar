...
import {API_PIC_CONFIG_ACCOUNT, API_PIC_CONFIG_PAT, API_PIC_IMAGE} from "@/constants/ApiConstant";
...
import type {Image} from "@/types/pic";
...
<template>
    <div class="container mx-auto min-h-screen flex flex-col">
            ...
            <div class="flex justify-center items-center space-x-4">
                ...
                <a-modal v-model:open="noticeModalOpen" @ok="hideNoticeModal">
                    ...
                    <ul class="list-disc">
                        ...
                        <li><span class="font-bold">使用限额：</span>为了保证Github资源的合理使用，每个Github账号只享有1000天的上传权限（到期后仍可查看历史图片，但无法新增）。</li>
        ...
        <div class="flex-1 border-2 rounded-2xl p-4 mb-4 flex flex-col">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 p-4" v-if="ownerName && images && !refreshing">
                <template v-for="image in images">
                    <div class="flex justify-center items-center mb-8">
                        <div class="p-3 rounded-2xl item-shadow">
...
<!--                                     class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 cursor-pointer" crossorigin="anonymous"/>-->
<!--                            </div>-->
                            <img :src="`${bestImageLinkGenerator.generate(ownerName, image.path)}`" :alt="image.path.split('/')[1]"
                                 class="w-64 h-48 object-cover mb-2 mx-auto" crossorigin="anonymous"/>
                            <div class="overflow-x-scroll whitespace-nowrap w-64 py-2 text-center">
...
[data-theme="dark"] .item-shadow {
    box-shadow: 5px 5px 10px #191e24, -5px -5px 10px #212830;
