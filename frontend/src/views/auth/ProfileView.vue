...
import { USER_AVATAR_PREFIX } from '@/constants/media';
import { getFileExtension } from '@/utils/file';
import type { R } from '@/types/common';
import SeasonStatsRadar from '@/views/kill/components/SeasonStatsRadar.vue';
import SeasonRankGrid from '@/views/kill/components/SeasonRankGrid.vue';
...
const exportDailyStats = () => {
    axios
        .get(`${API_KILL_STATS_DAILY}?dayNum=${dailyStatsDayNum.value}`, {
            responseType: 'blob',
        })
        .then(res => {
            const data = res.data;
            ...
            downloadBlob(
                data,
                `${userStore.nickname}的战绩信息(近${dailyStatsDayNum.value}日).xlsx`
            );
        });
};
...
const updateAvatar = async () => {
    ...
    // 获取后端提供的七牛云上传token
    const token: string = (await axios.get(API_COMMON_QINIU_UPLOAD)).data.data;
    // 设置上传任务
    ...
    observable.subscribe({
        ...
        complete: async res => {
            // 更新后端存储的头像文件名
            const filename = res.key.substring(USER_AVATAR_PREFIX.length + 1);
            const response: R<UserEditableInfo> = (
                await axios.patch(API_USER_INFO_AVATAR, { avatar: filename })
            ).data;
            if (response.code <= 0) {
...
const updateNickname = async () => {
    const response: R<UserEditableInfo> = (
        await axios.patch(API_USER_INFO_NICKNAME, { nickname: nickname.value })
    ).data;
    if (response.code <= 0) {
        errorMessage.value += response.msg;
    } else {
...
const updatePassword = async () => {
    const response: R<UserEditableInfo> = (
        await axios.patch(API_USER_INFO_PASSWORD, updatePasswordRequestVo)
    ).data;
    if (response.code <= 0) {
        errorMessage.value += response.msg;
    } else {
...
const updateEmail = async () => {
    const response: R<UserEditableInfo> = (
        await axios.patch(API_USER_INFO_EMAIL, updateEmailRequestVo)
    ).data;
    if (response.code <= 0) {
        errorMessage.value += response.msg;
    } else {
...
<template>
    <div class="p-4 w-full max-w-[50rem] mx-auto">
            ...
            <div class="overflow-scroll">
                ...
                <div class="flex items-center">
                    ...
                    <div class="flex-1 flex justify-center items-center space-x-6">
                        ...
                        <input
                            id="tentacles"
                            v-model="dailyStatsDayNum"
                            class="input w-[4rem] rounded-md"
                            max="90"
                            min="1"
                            name="tentacles"
                            type="number" />
        ...
        <div>
                    ...
                    <div class="flex-1 p-2 flex items-center justify-center space-x-4 mx-4">
                        <div class="avatar">
                            <div class="rounded-xl w-24">
                                <img
                                    :src="
                                        selectedAvatar == '' ? userStore.avatarLink : selectedAvatar
                                    "
                                    alt="头像"
            ...
            <button
                :disabled="selectedAvatar == '' && nickname == userStore.nickname"
                class="btn w-full"
        ...
        <div>
            ...
            <div class="overflow-scroll">
                <div class="flex items-center py-2">
                    <label class="field-label" for="password"> 更换密码 </label>
                    <div class="flex-1 flex">
                        <div class="w-1/2 px-4">
                            <div class="input input-bordered flex items-center space-x-2">
                                ...
                                <input
                                    id="password"
                                    v-model="updatePasswordRequestVo.newPassword"
                                ...
                                <font-awesome-icon
                                    :icon="
                                        hiddenNewPassword ? ['fas', 'eye-slash'] : ['fas', 'eye']
                                    "
                        ...
                        <div class="w-1/2 px-4">
                            <div class="input input-bordered flex items-center space-x-2">
                                <font-awesome-icon :icon="['fas', 'key']" />
                                <input
                                    v-model="updatePasswordRequestVo.confirmPassword"
                ...
                <div class="flex items-center py-2">
                    <label class="field-label" for="email"> 换绑邮箱 </label>
                    <div class="flex-1 flex">
                        <div class="w-1/2 px-4">
                            <div class="input input-bordered flex items-center space-x-2">
                                ...
                                <input
                                    id="email"
                                    v-model="updateEmailRequestVo.email"
                                    class="w-full"
                                    name="email"
                                    placeholder="请输入邮箱"
                                    type="text" />
