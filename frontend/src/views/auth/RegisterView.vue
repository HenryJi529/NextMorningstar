...
import { getClientGeolocationCoords } from '@/utils/client';
...
import { popupNotice } from '@/utils/notice';
...
const setRandomNicknameWithGeolocationCoords = async () => {
    ...
    formState.value.nickname = (
        await axios.get(
            `${API_USER_RANDOM_NICKNAME}?latitude=${coords.latitude}&longitude=${coords.longitude}`
        )
    ).data.data;
};
...
const register = async () => {
    ...
    const response: R<RegisterResponseVo> = (
        await axios.post(API_USER_AUTH_REGISTER, formState.value)
    ).data;
    console.log(response);
    if (response.code !== ResponseCode.SUCCESS) {
        errorMessage.value = response.msg;
        if (
            response.code === ResponseCode.CHECK_CODE_TIMEOUT ||
            response.code === ResponseCode.CHECK_CODE_ERROR
        ) {
    ...
    <standalone-form :error-message="errorMessage">
        ...
        <div class="flex items-center">
            ...
            <div class="input input-bordered flex-1 flex items-center space-x-2">
                ...
                <input
                    ...
                    placeholder="请输入密码" />
                <font-awesome-icon
                    :icon="hiddenPassword ? ['fas', 'eye-slash'] : ['fas', 'eye']"
        ...
        <div class="flex items-center">
            ...
            <div class="input input-bordered flex-1 flex items-center space-x-2">
                ...
                <input
                    ...
                    placeholder="请确认密码" />
                <font-awesome-icon
                    :icon="hiddenConfirmPassword ? ['fas', 'eye-slash'] : ['fas', 'eye']"
                    class="cursor-pointer"
        ...
        <div class="flex items-center justify-around">
            ...
            <button
                class="btn btn-ghost rounded-lg"
                type="button"
                @click="router.push({ name: 'auth-recover-initiate' })">
                找回密码
            </button>
        </div>
    </standalone-form>
