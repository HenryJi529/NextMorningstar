...
<template>
    <BaseLayout>
        <div class="flex-1 grid grid-cols-3 gap-8 grid-flow-row w-full" id="container">
            ...
            <div class="flex items-center justify-center">
                <BarChart :style="`height: ${containerWidth/3*0.55}px;` + `width: ${containerWidth/3*0.8}px`" v-if="containerWidth > 0"/>
            </div>
            <div class="flex items-center justify-center">
                <line-chart :style="`height: ${containerWidth/3*0.6}px;` + `width: ${containerWidth/3*0.8}px`" v-if="containerWidth > 0"/>
            </div>
            <div class="flex items-center justify-center">
                <ring-chart :style="`height: ${containerWidth/3*0.6}px;` + `width: ${containerWidth/3*0.9}px`" v-if="containerWidth > 0"/>
            </div>
            <div class="flex items-center justify-center">
                <matrix-chart :style="`height: ${containerWidth/3*0.6}px;` + `width: ${containerWidth/3*0.9}px`" v-if="containerWidth > 0"/>
            </div>
            <div class="flex items-center justify-center">
                <map-chart :style="`height: ${containerWidth/3*0.6}px;`" v-if="containerWidth > 0"/>
            </div>
            <div class="flex items-center justify-center">
                <word-cloud :style="`height: ${containerWidth/3*0.6}px;`" v-if="containerWidth > 0"/>
            </div>
        </div>
    </BaseLayout>
</template>
