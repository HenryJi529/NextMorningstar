...
import TabsComponentCContent from '@/views/practice/ui/TabsComponentC.vue';
...
const componentC = defineComponent({
    name: '组件C',
    setup() {
        return () => h(TabsComponentCContent);
    },
});
const currentComponent1 = shallowRef<Component>(componentA);
const componentList1 = [componentA, componentB, componentC];

const componentMap2: Record<string, Component> = {
    A: componentA,
    B: componentB,
    C: TabsComponentCContent,
};
const selectedTabName2 = getFirstParam(route.query['tab2'] as string[] | string);
const activeTabName2 = ref(
    Object.keys(componentMap2).includes(selectedTabName2) ? selectedTabName2 : 'A'
);
    ...
    <div>
        <div>
            ...
            <div class="flex justify-center items-center space-x-2">
                <template v-for="component in componentList1" :key="component.name">
                    <div
                        :class="{ 'text-blue-500': currentComponent1.name === component.name }"
        ...
        <div>
            ...
            <div class="flex justify-center items-center space-x-2">
                <template v-for="tabName in Object.keys(componentMap2)" :key="tabName">
                    <div
                        :class="{ 'text-blue-500': activeTabName2 === tabName }"
                        class="cursor-pointer p-2"
