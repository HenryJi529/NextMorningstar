...
const validateFileExt = (file: File) => {
    for(let extension of props.allowedExtensions){
...
watch(selectedFile, async () => {
    ...
    if (selectedFile.value.size > 1024 * 1024 * props.limitedSize) {
        message.warn({content: `文件大于${props.limitedSize}MB...`, class: 'ant-message-notice-custom', duration: 2});
...
<template>
    <div :class="isDragOver ? 'border-blue-300 bg-blue-200' : 'border-gray-500 bg-gray-200'"
        ...
        <input id="uploadInput" :accept="allowedExtensions.join(', ')" class="hidden" type="file"
