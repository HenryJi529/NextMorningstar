...
const createCylinder = (radiusTop: number, radiusBottom: number, height: number, color: string, positionY: number) => {
...
const loadFunnelChart = (chartDom: HTMLDivElement, cameraZ: number = 2.5, cameraY: number = 0.2) => {
    const animate = () => {
    ...
    for(let i = 0; i < 9; i++){
        // 添加物体
        const cylinder = createCylinder(1.3-0.1*i, 1.3-0.1*(i+1), 0.1, colors[i], (4-i) * 0.12)
        scene.add(cylinder);
    }
    animate();
    renderer.render(scene, camera);
...
const updateFunnelChart = (chartDom: HTMLElement) => {
    ...
    let sizes =  [0.04, 0.037, 0.035, 0.032, 0.03, 0.027, 0.025, 0.023, 0.02];
    let locations = [0.75, 0.5, 0.27, 0.09, -0.08, -0.24, -0.385, -0.51, -0.62];
    const sum = data.value.map(item => item.value).reduce((acc, cur) => acc + cur, 0);
    for(let i = 0; i < 9; i++){
        // 添加标签
        const ratio = i < 4 ? data.value[i].value/sum*100 : data.value[i].value/sum*10000
        ...
        const percent = (Math.round(ratio*Math.pow(10, fraction))/Math.pow(10, fraction)).toFixed(fraction);
        const name = RISK_LEVEL_CODE_NAME_MAP[Object.keys(RISK_LEVEL_CODE_NAME_MAP)[i]];
        const textContent = name + ' ' + percent + (i < 4 ? '%': '‱');
        const label = createLabel(textContent, chartDom.offsetWidth * sizes[i], '#fff', new THREE.Vector3(0, locations[i], 0));
        scene.add(label);
    }
