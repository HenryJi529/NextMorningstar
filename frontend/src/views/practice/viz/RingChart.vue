    ...
    setTimeout(() => {
        ...
        for (let i = 0; i < data.value.length; i++) {
            ...
            data.value.forEach(datum => {
                if (
                    currentValue < (datum.value as number) ||
                    (currentValue === (datum.value as number) &&
                        currentName.localeCompare(datum.name) > 0)
                ) {
                    rank++;
                }
            });
...
const calcRingChartLabelPolylinePoints = (
    angle: number,
    startRadius: number,
    endRadius: number,
    ...
    return (
        ...
        ',' +
        (getPoint(endRadius, angle).y +
            (angle < 90 || angle > 270 ? -verticalLength : verticalLength))
    );
};

const getRingParams = (
    offsetX: number,
    polylineStartRadius: number,
    polylineMaxRadius: number,
    polylineHorizonLength: number,
    textFontSize: number,
    textXPadding: number,
    permyriadIndex: number
) => {
    interface Param {
        textContent: string;
        textFontSize: number;
    ...
    const textVerticalOffset = textFontSize * 0.3;
    const textHorizonOffset = textFontSize * 0.58;
    ...
    for (let i = 0; i < data.value.length; i++) {
        // 计算polylineAngle
        const currentAngle =
            ((data.value.length - (data.value[i].rank || 0)) /
                ((data.value.length * (data.value.length + 1)) / 2)) *
            360;
        ...
        const ratio =
            i < permyriadIndex
                ? ((data.value[i].value || 0) / valueSum) * 100
                : ((data.value[i].value || 0) / valueSum) * 10000;
        ...
        const textContent =
            (Math.round(ratio * Math.pow(10, fraction)) / Math.pow(10, fraction)).toFixed(
                fraction
            ) + (i < permyriadIndex ? '%' : '‱');
        ...
        const textWidth =
            (i >= permyriadIndex &&
            Bowser.parse(window.navigator.userAgent)['os']['name'] !== 'macOS'
                ? textContent.length + 1.5
                : textContent.length) * textHorizonOffset;

        // 计算polylineEndRadius
        let polylineHorizonProjectLength;
        if (polylineAngle < 90) {
            polylineHorizonProjectLength =
                Math.cos(((90 - polylineAngle) * Math.PI) / 180) * polylineHorizonLength;
        } else if (polylineAngle < 180) {
            polylineHorizonProjectLength =
                Math.cos(((polylineAngle - 90) * Math.PI) / 180) * polylineHorizonLength;
        } else if (polylineAngle < 270) {
            polylineHorizonProjectLength =
                Math.cos(((270 - polylineAngle) * Math.PI) / 180) * polylineHorizonLength;
        } else {
            polylineHorizonProjectLength =
                Math.cos(((polylineAngle - 270) * Math.PI) / 180) * polylineHorizonLength;
        }
        ...
        if (polylineAngle < 90) {
            textX =
                50 +
                polylineEndRadius * Math.sin((polylineAngle * Math.PI) / 180) +
                polylineHorizonLength +
                textXPadding +
                offsetX;
            textY =
                50 -
                polylineEndRadius * Math.cos((polylineAngle * Math.PI) / 180) +
                textVerticalOffset;
        } else if (polylineAngle < 180) {
            textX =
                50 +
                polylineEndRadius * Math.sin(((180 - polylineAngle) * Math.PI) / 180) +
                polylineHorizonLength +
                textXPadding +
                offsetX;
            textY =
                50 +
                polylineEndRadius * Math.cos(((180 - polylineAngle) * Math.PI) / 180) +
                textVerticalOffset;
        } else if (polylineAngle < 270) {
            textX =
                50 -
                polylineEndRadius * Math.sin(((polylineAngle - 180) * Math.PI) / 180) -
                polylineHorizonLength -
                textXPadding -
                textWidth +
                offsetX;
            textY =
                50 +
                polylineEndRadius * Math.cos(((polylineAngle - 180) * Math.PI) / 180) +
                textVerticalOffset;
        } else {
            textX =
                50 -
                polylineEndRadius * Math.sin(((360 - polylineAngle) * Math.PI) / 180) -
                polylineHorizonLength -
                textXPadding -
                textWidth +
                offsetX;
            textY =
                50 -
                polylineEndRadius * Math.cos(((360 - polylineAngle) * Math.PI) / 180) +
                textVerticalOffset;
        }
...
const calcConicGradient = () => {
    ...
    for (const datum of data.value) {
        ...
        const endPercent =
            currentPercent +
            ((data.value.length - (datum.rank || 0)) /
...
<template>
    <div class="flex items-center justify-center relative" ref="containerRef">
        <div
            class="center rounded-full"
            :style="
                `height: ${containerHeight * 0.64}px;` +
                `width: ${containerHeight * 0.64}px;` +
        ...
        <div
            class="center rounded-full"
            :style="
                `height: ${containerHeight * 0.58}px;` +
                `width: ${containerHeight * 0.58}px;` +
            ...
            "></div>
        <div
            v-if="data.filter(datum => datum.value !== undefined).length > 0"
            class="center rounded-full"
            :style="
            ...
            "></div>
        <div
            class="center bg-blue-200 rounded-full flex items-center justify-center"
            :style="`height: ${containerHeight * 0.29}px; width: ${containerHeight * 0.29}px;border-width: ${containerHeight * 0.01}px`"
            style="
                border-color: #124a62;
                background: linear-gradient(to bottom, #0d6b87, #122d43 45%, #122d43 55%, #0d6b87);
            ">
            <div class="text-white" :style="`font-size: ${containerHeight * 0.045}px;`">
                ...
                <div
                    class="text-center"
                    v-text="
                        data.filter(datum => datum.value !== undefined).length > 0
                            ? Number(
        ...
        <div
            ...
            <svg :viewBox="`0 0 ${128} 100`" class="w-full h-full">
                <template
                    v-for="(param, ind) in getRingParams(
                        (128 - 100) / 2,
                        0.5 * 50,
                        44,
                        5,
