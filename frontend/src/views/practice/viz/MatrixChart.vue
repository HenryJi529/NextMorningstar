...
const renderMatrixItem = (chartDom: HTMLDivElement, _: echarts.CustomSeriesRenderItemParams, api: echarts.CustomSeriesRenderItemAPI) => {
...
const loadChart = (chartDom: HTMLDivElement) => {
    ...
    const option = {
        ...
        series: [
            {
                type: 'custom',
                'renderItem': (_: echarts.CustomSeriesRenderItemParams, api: echarts.CustomSeriesRenderItemAPI) => {
    ...
    const toggleSeries = () => {
        ...
        myChart.setOption({
            series: [
                {},
                { data: isShow ? option.series[1].data : [] },
                {}
            ]
        });
        clearInterval(timer);
        timer = setInterval(toggleSeries, isShow ? visiblePeriod : invisiblePeriod);
    }
    let timer = setInterval(toggleSeries, isShow ? visiblePeriod : invisiblePeriod);
}
    ...
    <div class="h-full w-full relative flex justify-center items-center">
        ...
        <div class="absolute left-0 flex h-full flex-col w-[24%]">
            ...
            <div class="h-[71%] w-full flex items-center">
                <div class="vertical-text text-center font-bold text-white"
                ...
                <div class="h-full flex flex-col items-end justify-around text-white whitespace-nowrap" :style="`font-size: ${containerWidth*0.025}px;`">
                    <div>高风险</div>
                    <div>较高风险</div>
                    <div>中风险</div>
                    <div>中低风险</div>
        ...
        <div class="absolute bottom-0 w-full flex h-[15%]">
            ...
            <div class="w-[61%] flex flex-col justify-end">
                ...
                <div class="flex items-center justify-around text-white" :style="`font-size: ${containerWidth*0.025}px;`">
                ...
                <div class="text-center font-bold text-white" :style="`letter-spacing: ${containerWidth*0.01}px; font-size: ${containerWidth*0.03}px;`">
                    控制有效性
                </div>
            </div>
            <div class="w-[15%]"></div>
        ...
        <div class="absolute right-0 flex flex-col h-full">
            ...
            <div class="h-[71%] w-full flex items-start">
                <div class="flex flex-col items-center" :style="`margin-right: ${containerWidth*0.03}px`">
                    <img :src="StarIcon" alt="2021" :style="`height: ${containerHeight*0.07}px;` + `margin-bottom: ${containerHeight*0.03}px`" />
                    <div class="vertical-text text-white" :style="`letter-spacing: ${containerHeight*0.01}px; font-size: ${containerHeight*0.043}px;`">2021年整体剩余风险</div>
                </div>
                <div class="flex flex-col items-center">
                    <img :src="LogoIcon" alt="2021" :style="`height: ${containerHeight*0.07}px;` + `margin-bottom: ${containerHeight*0.03}px`" />
                    <div class="vertical-text text-white" :style="`letter-spacing: ${containerHeight*0.01}px; font-size: ${containerHeight*0.043}px;`">2024年整体剩余风险</div>
                </div>
            </div>
            <div class="h-[26%]"></div>
        </div>
