...
const fetchData = () => {
    ...
    setTimeout(()=>{
        ...
        const data1 = rawData.map(rawDatum => {
            const datum: Datum = {
                ...rawDatum,
                isActive: false
            }
...
<template>
    <div ref="containerRef" class="w-full flex items-center" :style="`height: ${containerWidth * 0.5}px`">
        <div class="flex-1 relative w-full h-full">
            <img :src="LOGO_PATH" alt="logo" class="center" :style="`height: ${containerWidth * 0.24}px`" />
            <svg width="100%" height="100%" viewBox="0 0 140 100">
                <defs>
                    <template v-for="(_, ind) in data">
                        <radialGradient :id="`grad${ind}`" cx="50%" cy="50%" r="50%">
                            <stop offset="0%" :style="{ stopColor: COLOR_PAIR_LIST[ind % COLOR_PAIR_LIST.length].start, stopOpacity: 1 }"/>
                            <stop offset="100%" :style="{ stopColor: COLOR_PAIR_LIST[ind % COLOR_PAIR_LIST.length].end, stopOpacity: 1 }"/>
                        </radialGradient>
                    </template>
                </defs>
                <template v-for="(datum, ind) in data">
                    <path class="cursor-pointer" @pointerenter="activate(ind)"
                          :d="getNormalSectorPath(datum.isActive)" :fill="`url(#grad${ind})`"
                          :transform="`translate(20, 0) rotate(${-30 + (360/data.length/2) * (2*ind+1)}, 50, 50)`">
                    </path>
                    <text v-if="datum.breakpoint" class="pointer-events-none cursor-pointer" fill="white"
                          :font-size="getLabelParams(ind).fontSize" :x="getLabelParams(ind).x" :y="getLabelParams(ind).y"
                          :transform="`translate(20, 0) rotate(${getLabelParams(ind).rotateAngle}, 50, 50)`">
                        <tspan :x="getLabelParams(ind).x" dy="-0.5em" v-text="datum.name.slice(0, datum.breakpoint)"></tspan>
                        <tspan :x="getLabelParams(ind).x" dy="1.1em" v-text="datum.name.slice(datum.breakpoint)"></tspan>
                    </text>
                    <text v-else class="pointer-events-none cursor-pointer" fill="white"
                          :font-size="getLabelParams(ind).fontSize" :x="getLabelParams(ind).x" :y="getLabelParams(ind).y"
                          :transform="`translate(20, 0) rotate(${getLabelParams(ind).rotateAngle}, 50, 50)`"
                          v-text="datum.name">
                    </text>
                </template>
                <circle v-if="data.filter(item => item.isActive).length === 2" :class="{'invisible': data.filter(item => item.isActive)[0].value === 0 }"
                        :cx="getLineParams().startX1" :cy="getLineParams().startY1" r="2" fill="white" stroke="#ffffff80" stroke-width="1"/>
                <circle v-if="data.filter(item => item.isActive).length === 2" :class="{'invisible': data.filter(item => item.isActive)[1].value === 0 }"
                        :cx="getLineParams().startX2" :cy="getLineParams().startY2" r="2" fill="white" stroke="#ffffff80" stroke-width="1"/>
                <line v-if="data.filter(item => item.isActive).length === 2" :class="{'invisible': data.filter(item => item.isActive)[0].value === 0 }"
                      :x1="getLineParams().startX1" :y1="getLineParams().startY1" x2="140" y2="17.6" stroke="white" stroke-width="1"/>
                <line v-if="data.filter(item => item.isActive).length === 2" :class="{'invisible': data.filter(item => item.isActive)[1].value === 0 }"
                      :x1="getLineParams().startX2" :y1="getLineParams().startY2" x2="140" y2="82.4" stroke="white" stroke-width="1" />
            </svg>
        </div>
        <div class="w-[30%] h-full">
            <div v-if="data.filter(item => item.isActive).length > 0" class="h-full flex flex-col" :class="data.filter(item => item.isActive).length ===2 ? 'justify-between' : 'justify-center'">
                <div v-for="n in Array.from({ length: data.filter(item => item.isActive).length }, (_, i) => i)" class="w-full" :style="`height: ${containerWidth * 0.176}px`">
                    <div class="h-full w-full relative" :class="{'invisible': data.filter(item => item.isActive)[n].value === 0}">
                        ...
                        <div class="absolute top-0 left-0 w-full h-full flex flex-col justify-between"
                            ...
                            <div class="font-bold" :style="`font-size: ${containerWidth*0.028}px; line-height: ${containerWidth*0.03}px`">
                                <div v-if="data.filter(item => item.isActive)[n].breakpoint">
                                    <div v-text="data.filter(item => item.isActive)[n].name.slice(0, data.filter(item => item.isActive)[n].breakpoint)"></div>
                                    <div v-text="data.filter(item => item.isActive)[n].name.slice(data.filter(item => item.isActive)[n].breakpoint)"></div>
                                </div>
                                <div v-else v-text="data.filter(item => item.isActive)[n].name">
                                </div>
                            </div>
                            <div class="flex justify-between items-center w-full">
                                <div :style="`font-size: ${containerWidth * 0.022}px`">预警量</div>
                                <div v-text="data.filter(item => item.isActive)[n].value"
                                     :style="`font-size: ${containerWidth * 0.06}px; line-height: ${containerWidth * 0.06}px`"
                                    style="color: transparent; background-image: linear-gradient(to bottom, #ffffff, #008BE6); background-clip: text; -webkit-background-clip: text">
                                </div>
                            </div>
                        </div>
                    </div>
