...
import * as api from '@/axios/practice';
...
const parseInFrontend = () => {
    ...
    reader.onload = function (e: ProgressEvent<FileReader>) {
        ...
        console.log('前端解析结果', jsonData); // 在控制台输出解析的数据
    };
...
const parseInBackend = async () => {
    ...
    const startTime = new Date();
    const response = (await api.parseExcel(selectedFile.value)).data;
    const endTime = new Date();
    backendParseDuration.value = response.data.duration;
    backendTransferDuration.value =
        endTime.getTime() - startTime.getTime() - response.data.duration;
    console.log('后端解析结果', response.data.data);
};
    ...
    <base-layout>
        <div>
            ...
            <div v-if="frontendDuration && backendParseDuration && backendTransferDuration">
                ...
                <div>
                    后端解析耗时: {{ backendTransferDuration }}ms(传输),
                    {{ backendParseDuration }}ms(解析)
                </div>
            </div>
