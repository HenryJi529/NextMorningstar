...
const fetchPersonList = (name: string) => {
    return new Promise(resolve => {
        axios.get(`/practice/small-world-network/find-persons-by-name?name=${name}`).then(res => {
...
const search = () => {
    ...
    axios({
        method: 'get',
        url: '/practice/small-world-network/find-shortest-path-between-two-persons',
        params: {
        ...
        node: {
            ...
            style: {
                ...
                labelFontWeight: datum =>
                    datum.id === 'P' + person1.value?.id || datum.id === 'P' + person2.value?.id
                        ? 'bold'
                        : 'normal',
            ...
            <div class="flex-1 flex items-center">
                ...
                <div class="relative flex-1">
                    <input
                        ...
                        v-model="person1Name"
                        class="person-input"
                        @change="fetchPersonList1"
                        @focus="select1Visible = true" />
                    <div class="absolute w-full mt-1 z-10" v-show="select1Visible">
                        <template v-for="person in personList1.slice(0, 10)" :key="person.id">
                            <div class="person-item" @click="choosePerson1(person)">
            ...
            <div class="flex-1 flex items-center">
                ...
                <div class="relative flex-1">
                    <input
                        ...
                        v-model="person2Name"
                        class="person-input"
                        @change="fetchPersonList2"
                        @focus="select2Visible = true" />
                    <div class="absolute w-full mt-1 z-10" v-show="select2Visible">
                        <template v-for="person in personList2.slice(0, 10)" :key="person.id">
                            <div class="person-item" @click="choosePerson2(person)">
                                {{ person.name }} ({{ person.born }})
                            </div>
                        </template>
                    </div>
