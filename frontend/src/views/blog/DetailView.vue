...
import {
    API_BLOG_ARTICLE_COMMENT,
    API_BLOG_ARTICLE_DETAIL,
    API_BLOG_ARTICLE_SUMMARY,
    API_COMMON_QRCODE,
} from '@/constants/api';
import { onMounted, ref, watch } from 'vue';
import ArticleSection from '@/views/blog/components/ArticleSection.vue';
import CommentSection from '@/views/blog/components/CommentSection.vue';
import CommonGuestArea from '@/views/blog/components/CommonGuestArea.vue';
import TocSection from '@/views/blog/components/TocSection.vue';
import { useMarked, type TocTreeNode } from '@/composables/marked';
const { convertMarkdownToHtml, tocHandler } = useMarked(true);
...
const loadArticle = async () => {
    ...
    const response: R<ArticleDetail> = (
        await axios.get(API_BLOG_ARTICLE_DETAIL.replace('{id}', id))
    ).data;
    ...
    articleBody.value = await convertMarkdownToHtml(articleDetail.value.article.content);
...
const loadSummary = async () => {
    const id = route.params.id as string;
    const url = `${API_BLOG_ARTICLE_SUMMARY.replace('{id}', id)}`;
    const response: R<string> = (await axios.get(url)).data;
    articleSummary.value = response.data;
};
...
const shareByQrcode = async () => {
    if (articleQrcode.value == '') {
        const response: R<string> = (await axios.get(`${API_COMMON_QRCODE}?data=${location.href}`))
            .data;
        if (response.code !== ResponseCode.SUCCESS) {
            return;
        }
        ...
        <template #custom-main-part>
            <div>
                <div v-if="articleNotExist" class="h-[30rem] flex justify-center items-center">
                    <div class="text-3xl">文章不存在</div>
                </div>
                <div v-else class="p-6">
                    <div v-if="articleDetail && articleBody" class="relative">
                        <div
                            v-if="hasAnyPermission([Permission.BLOG_ARTICLE_MANAGE])"
                            class="absolute right-0 text-2xl">
                            <router-link
                                :to="{
                                    name: 'blog-manage-article-edit',
                                    params: { id: articleDetail.article.id },
                                }">
                        ...
                        <div class="flex justify-center space-x-20 my-6">
                            <div
                                class="btn h-[5rem] w-[5rem] rounded-full text-4xl"
                                @click="shareByLink">
                        ...
                        <div v-if="sharedByLink">
                            <alert-success> 已复制到剪贴板 </alert-success>
                        </div>
                    ...
                    <div v-if="comments && articleDetail">
                        <comment-section
                            :article-id="articleDetail.article.id"
                            :comments="comments"
