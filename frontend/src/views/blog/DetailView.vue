...
import {getMarked, getTocRender, TocHandler, type TocTreeNode} from "@/utils/handleMarked";
...
const loadArticle = async () => {
    ...
    articleBody.value = await marked.parse(articleDetail.value.article.content);
...
const loadComments = async () => {
    ...
    for(let i=0; i < comments.value.length; i++){
        commentBodyArray.value.push(await marked.parse(comments.value[i].content));
    }
}
    ...
    <main-body>
        ...
        <template #custom-side-bar>
            <div v-if="tocTree && tocTree.length > 0" class="article-toc">
                <div class="text-xl pb-2">目录</div>
                <toc-section :toc-tree="tocTree"/>
                <div class="divider"></div>
            </div>
