...
const parseQueryParams = () => {
    ...
    if ('categoryId' in route.query) {
        queryParams.categoryId = parseInt(
            getFirstParam(route.query.categoryId as string[] | string)
        );
    }
            ...
            <div v-if="pageResult" class="h-full">
                <div v-if="pageResult.currentPageSize > 0" class="h-full px-4 flex flex-col">
                    ...
                    <div class="py-4 flex flex-col space-y-4 flex-1">
                        <template
                            v-for="articleDetail in pageResult.records"
                            ...
                            <div class="overflow-hidden w-full">
                                <div
                                    ...
                                    ">
                                    <div
                                        class="text-center text-2xl hover:text-primary"
                                ...
                                <article-info-bar
                                    ...
                                    class="text-sm mb-1" />
                                <div
                                    class="indent-8 break-words"
                                    :class="{ 'select-none': !userStore.isAuthenticated }"
                                    v-html="articleDetail.article.content"></div>
                                <div
                                    class="text-center flex justify-center items-center space-x-2 cursor-pointer mt-2"
                                    @click="
                                        router.push({
                                            name: 'blog-detail',
                                            params: { id: articleDetail.article.id },
                    ...
                    <div v-if="pageResult" class="flex justify-between text-xl p-2 px-4 pb-10">
                        <div
                            :class="{ invisible: pageResult.pageNum == 1 }"
                        ...
                        <div
                            :class="{ invisible: pageResult.pageNum == pageResult.totalPageNum }"
                            class="cursor-pointer flex items-center space-x-1"
                            @click="viewNextPage">
