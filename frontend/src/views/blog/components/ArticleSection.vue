...
import ArticleInfoBar from '@/views/blog/components/ArticleInfoBar.vue';
import { generateCodeBlockCopyButton } from '@/composables/marked';
...
<template>
    <article>
        ...
        <article-info-bar :article-detail="articleDetail" class="text-base pb-8" />
        <div
            v-show="isImageModalOpen"
            class="w-screen h-page top-0 left-0 z-10 bg-gray-800/90 flex items-center justify-center">
            <div
                class="rounded-full absolute right-8 top-8 cursor-pointer z-20"
                @click="isImageModalOpen = false">
                <font-awesome-icon
                    :icon="['fas', 'xmark']"
                    class="text-4xl text-stone-200 dark:text-stone-700" />
            </div>
            <img :src="selectedImageLink" alt="大图" />
        </div>
        <div class="border-2 border-dashed dark:border-blue-200 border-blue-600 rounded-xl p-4 min-h-36 mb-6 mx-12">
            <div class="text-xl dark:text-blue-400 text-blue-700 mb-4">AI 摘要：</div>
            <div class="indent-8" v-if="articleSummary">{{ articleSummary }}</div>
            <div class="flex justify-center items-center" v-else>
                <span class="loading loading-dots loading-lg"></span>
            </div>
        </div>
        <div
            :class="{ 'select-none': !userStore.isAuthenticated }"
            class="article-body"
            v-html="articleBody"></div>
    </article>
</template>
