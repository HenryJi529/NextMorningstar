...
import { API_BLOG_CATEGORY_ALL, API_BLOG_TAG_ALL } from '@/constants/api';
...
<template>
    <div class="flex flex-col space-y-4">
        <div class="flex space-x-16 items-center">
            ...
            <input id="title" v-model="data.title" class="admin-manage-input" type="text" />
        </div>
        <div class="flex space-x-16 items-center">
            <label class="font-bold w-32" for="content">Content</label>
            <textarea
                id="content"
        ...
        <div class="flex space-x-16 items-center">
            ...
            <div class="flex-1 flex items-center">
                <select id="category" v-model="data.categoryId" class="admin-manage-input">
                    ...
                    <template v-for="categoryDetail in categoryDetails" :key="categoryDetail.id">
            ...
            <div v-if="tagDetails" class="flex-1">
                <div class="w-full flex items-center">
                    <div class="admin-manage-input flex items-center flex-wrap justify-start">
                        <template v-for="tagId in data.tagIds" :key="tagId">
                            <div
                                ...
                                <span>{{
                                    tagDetails.filter(tagDetail => tagDetail.id == tagId)[0].name
                                }}</span>
                            </div>
                        </template>
                        <input
                            v-model="tagPrefix"
                            class="bg-transparent px-2 py-1 rounded-lg w-20 ml-1"
                            placeholder="请选择..."
                            type="text"
                ...
                <div v-if="isTagDropdownOpen" class="flex items-center flex-wrap pl-2 pr-8 pt-1">
                    <template
                        v-for="tagDetail in tagDetails.filter(tag => {
                            return (
                                (tagPrefix == '' ||
                                    (tagPrefix != '' &&
                                        tag.name
                                            .toLowerCase()
                                            .includes(tagPrefix.toLowerCase()))) &&
