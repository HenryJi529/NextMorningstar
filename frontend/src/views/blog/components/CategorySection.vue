    ...
    <div>
        ...
        <div v-if="categories">
            <div v-if="categories.length > 0" class="grid grid-cols-2 lg:grid-cols-1">
                <template v-for="categoryDetail in categories" :key="categoryDetail.id">
                    <div
                        v-if="categoryDetail.count > 0"
                        class="flex space-x-2 items-center px-2 py-1">
                        <font-awesome-icon :icon="['fas', 'folder']" />
                        <div
                            class="hover:text-orange-300 cursor-pointer"
                            @click="jumpToCategory(categoryDetail.id)">
                            <span class="text-xl lg:text-base">
                                {{ categoryDetail.name
                                }}<sup class="text-gray-500">{{ categoryDetail.count }}</sup>
                            </span>
                        </div>
                    </div>
                </template>
