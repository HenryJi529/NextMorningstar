...
const getRoseParams = (containerRadius: number) => {
    ...
    for (const datum of data.value) {
        ...
        const alpha = 0.85;
        const sectorRadius =
            alpha * containerRadius +
            ((1 - alpha) * containerRadius * (maxValue - datum.value)) / maxValue; // 数值越小半径越大
        ...
        const { x: textX, y: textY } = getPoint(
            containerRadius,
            sectorRadius * 0.6,
...
<template>
    <div class="w-full h-full flex items-center justify-center" ref="containerRef">
        <div
            class="relative"
            :style="{ width: containerHeight + 'px', height: containerHeight + 'px' }">
            <div v-for="(param, index) in getRoseParams(containerHeight / 2)" :key="index">
                <div
                    class="absolute w-full h-full rounded-full"
                    :style="
                        `background: radial-gradient(circle at center, ${param.startColor}, ${param.endColor});` +
                        `clip-path: url(#customClipPath-${index});`
                    "></div>
                <div
                    class="absolute font-bold text-black"
                    v-text="param.value"
                    :style="
                        `transform: translate(-50%, -50%);` +
                        `font-size: ${containerHeight * 0.06}px;` +
                        `left: ${param.textX}px;` +
                        `top: ${param.textY}px`
                    "></div>
                <svg width="0" height="0" class="absolute">
                    <defs>
                        <clipPath :id="'customClipPath-' + index">
                            <path
                                :d="`M ${param.startX} ${param.startY} L ${containerHeight / 2} ${containerHeight / 2} L ${param.endX} ${param.endY} A ${param.sectorRadius} ${param.sectorRadius} 0 0 0 ${param.startX} ${param.startY}`" />
                        </clipPath>
                    </defs>
                </svg>
            </div>
        ...
        <div
            class="w-[30%] flex flex-col space-y-[0.5em] ml-[1em] mr-[2em] py-[1.5em]"
            :style="`font-size: ${containerHeight * 0.06}px`"
            style="
                background-color: #10283f;
                border-radius: 1em;
                box-shadow: inset 0 0 1em #419ce1;
            ">
            <div
                v-for="(datum, index) in data"
                :key="index"
                class="flex items-center justify-center space-x-[0.5em]">
                <div
                    class="rounded-full w-[0.7em] h-[0.7em]"
                    :style="
