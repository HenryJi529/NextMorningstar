...
<template>
    <div @click="handlePrint" class="absolute right-0 top-0 p-10 text-6xl" v-if="isDesktop()">
        <font-awesome-icon :icon="['fas', 'download']" class="cursor-pointer"/>
    </div>
    <div ref="page" id="page" class="box-border px-8 py-12 mx-auto font-family-song" data-theme="light">
        <div class="flex justify-between items-end gap-10">
            ...
            <div class="w-[8rem]">
                <img src="../../assets/img/resume/证件照.JPG" alt="证件照" class="w-full">
            </div>
        </div>
                    ...
                    <div class="pl-8">
                        <div><span class="font-bold">主要工作：</span>
                            <ol class="ml-8 list-disc">
                                ...
                                <li>将图片的上传与更新转换为Git仓库中commit的提交，响应的文件路径通过jsDelivr封装，无缝提供CDN支持</li>
                                <li>实现图片压缩功能，支持自定义压缩质量，默认采用WebP格式，兼容JPEG格式作为备选方案</li>
                                <li>提供Web界面管理图片资源，通过缓存每日图片列表与按响应时间选择CDN等方法提升访问速度</li>
    ...
    <morningstar-footer class="pt-2" v-if="isDesktop()"/>
</template>
