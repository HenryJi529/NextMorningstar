...
const updateItem = async () => {
    const response: R<object> = (await axios.patch(endpoint, props.data)).data;
...
const deleteItem = async () => {
    const response: R<object> = (await axios.delete(endpoint)).data;
    if (response.code !== ResponseCode.SUCCESS) {
        alert(response.msg);
        return;
    }
...
<template>
    <div>
        <div v-if="itemNotExist">{{ props.resource }} 不存在</div>
