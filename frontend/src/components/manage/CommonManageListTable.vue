...
const deleteSelected = () => {
    ...
    const deletePromises = selectIds.map(id => {
        return axios.delete(api.replace('{id}', id.toString()));
    });
...
<template>
    <div class="flex justify-between items-center">
        ...
        <div class="flex justify-end items-center space-x-4 p-4">
            ...
            <div
                :class="{ 'btn-disabled': !deleteApi }"
                class="btn rounded-xl btn-secondary"
    ...
    <div class="overflow-scroll">
        <table
            class="w-full border-8 border-color rounded-xl border-solid border-separate border-spacing-0">
            <thead>
                <tr class="flex">
                    <th data-col="0">
                    ...
                    <template
                        v-for="(itemConfig, ind) in config.itemConfigs"
                        :key="itemConfig.name">
                        <th
                            :class="[
                                {
                                    'text-center': itemConfig.center,
                                    'text-left': !itemConfig.center,
                                },
                                'flex',
                                'items-center',
                                'justify-center',
                            ...
                            class="relative">
                            <span
                                v-if="itemConfig.sortable"
                                :class="{ 'cursor-pointer': !includeOrder(itemConfig.name) }"
                                @click="setDefaultOrder(itemConfig.name)"
                                >{{ itemConfig.alias ?? itemConfig.name }}</span
                            >
                            <span v-else>{{ itemConfig.name }}</span>

                            <div v-if="itemConfig.sortable">
                                <div
                                    class="absolute right-3 top-2"
                                    @click="switchOrder(itemConfig.name)">
                                    <font-awesome-icon
                                        v-if="sortOrders.includes(`${itemConfig.name}_asc`)"
                                        :icon="['fas', 'arrow-down-1-9']"
                                        class="cursor-pointer" />
                                    <font-awesome-icon
                                        v-if="sortOrders.includes(`${itemConfig.name}_desc`)"
                                        :icon="['fas', 'arrow-down-9-1']"
                                        class="cursor-pointer" />
                                </div>
                                <div
                                    v-if="includeOrder(itemConfig.name)"
            ...
            <tbody>
                <template v-for="item in sortedItems" :key="JSON.stringify(item)">
                    <tr class="flex">
                        <td class="flex items-center" data-col="0">
                            <input
                                :checked="checkAll || checkedIds.includes(item.id)"
                                type="checkbox"
                        ...
                        <template
                            ...
                            :key="itemConfig.name">
                            <td
                                :class="[itemConfig.class, 'flex', 'items-center', 'flex-nowrap']"
                                :data-col="ind + 1">
                                <div
                                    :class="{
                                        'text-center': itemConfig.center,
                                        'cursor-pointer text-blue-500 dark:text-blue-400':
                                            itemConfig.name === config.identityField,
                                    }"
                                    class="w-full overflow-scroll"
                                    @click="
                                        itemConfig.name === config.identityField &&
                                        router.push({
                                            name: `${app}-manage-${resource}-edit`,
                                            params: { id: item.id },
                                        })
                                    ">
