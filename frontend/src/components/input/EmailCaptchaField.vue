...
import CountDown from '@/components/input/CountDown.vue';
import { ref } from 'vue';
import type { R } from '@/types/common';
import axios from '@/axios';
import { API_COMMON_CAPTCHA_EMAIL } from '@/constants/api';
...
const getEmailCaptcha = async () => {
    ...
    const response: R<object> = (
        await axios.get(`${API_COMMON_CAPTCHA_EMAIL}?email=${props.data.email}`)
    ).data;
