...
<template>
    <div class="flex justify-between items-center">
        ...
        <div class="flex justify-end items-center space-x-4 p-4">
            <div class="btn rounded-xl btn-primary" @click="router.push({name: `${app}-manage-${resource}-add`})" :id="`${app}-${resource}-add-button`">
                <font-awesome-icon :icon="['fas', 'circle-plus']" />新增
            </div>
            <div class="btn rounded-xl btn-secondary" :class="{'btn-disabled': !deleteApi}" @click="deleteSelected">
    ...
    <table class="w-full">
        ...
        <tr class="flex">
            ...
            <template v-for="(itemConfig, ind) in config.itemConfigs">
                <th class="relative" :class="[{'text-center': itemConfig.center, 'text-left': !itemConfig.center}, itemConfig.width]" :data-col="ind+1">
        ...
        <template v-for="item in sortedItems">
            <tr class="flex">
                ...
                <template v-for="(itemConfig, ind) in config.itemConfigs">
                    <td :class="[itemConfig.width, 'flex', 'items-center', 'justify-center', {'text-center': itemConfig.center, 'text-left': !itemConfig.center}]" :data-col="ind+1">
                        <div v-if="itemConfig.name === config.identityField" class="cursor-pointer text-blue-500 dark:text-blue-400"
                             @click="router.push({name: `${app}-manage-${resource}-edit`, params: {id: item.id }})">
