    ...
    <div class="flex flex-col space-y-4">
        <div class="flex space-x-16 items-center">
            ...
            <input id="title" v-model="data.title" type="text"
                   class="blog-manage-input flex-1 p-2 rounded-lg"/>
        </div>
        <div class="flex space-x-16 items-start">
            <label for="content" class="font-bold w-32 pt-4">Content</label>
            <textarea id="content" v-model="data.content" rows="5" class="blog-manage-input flex-1 p-2 rounded-lg"></textarea>
        </div>
        <div class="flex space-x-16 items-center">
            <label for="category" class="font-bold w-32">Category</label>
            <div class="flex-1 flex items-center">
                <select id="category" v-model="data.categoryId" class="blog-manage-input flex-1 p-2 rounded-lg">
        ...
        <div class="flex space-x-16 items-center">
            ...
            <div class="flex-1" v-if="tagDetails">
                <div class="w-full flex items-center">
                    <div class="blog-manage-input flex-1 py-1 px-2 rounded-lg flex items-center flex-wrap justify-start">
                ...
                <div v-if="openTagDropdown" class="flex items-center flex-wrap pl-2 pr-8 pt-1">
                    <template v-for="tagDetail in tagDetails.filter(tag => {
                        return (tagPrefix == '' || (tagPrefix != '' && tag.name.includes(tagPrefix))) && !data.tagIds.includes(tag.id)
                    })">
                        <div @click="data.tagIds.push(tagDetail.id)"
                             class="mx-1 my-[1px] border-gray-400 dark:border-gray-700 px-2 py-1 rounded-lg border-[1px] bg-white dark:bg-teal-700 hover:bg-blue-400 cursor-pointer">
                            {{ tagDetail.name }}
                        </div>
                    </template>
                </div>
