...
<template>
    <div class="flex flex-wrap justify-center space-x-2">
        <div v-if="articleDetail.category" class="flex space-x-1 items-center" @click="router.push({name: 'blog-list', query: {categoryId: articleDetail.category.id}})">
        ...
        <div class="flex space-x-1 items-center" v-if="articleDetail.tags.length > 0">
            ...
            <div>
                <template v-for="(tag, index) in articleDetail.tags">
                    ...
                    <span class="cursor-pointer hover:text-primary" @click="router.push({name: 'blog-list', query: {tagId: tag.id}})" v-html="tag.name"></span>
                </template>
            </div>
        </div>
