...
const lovePhotoData: LovePhotoData = (await axios.get(API_LOVE_RANDOM)).data;
...
const targetNum = isMobile ? totalNum/32 : totalNum/4;
...
function createEllipse(currentCx, currentCy, currentLevel) {
    ...
    ellipse.setAttribute("rx", getCurrentRx(currentLevel));
    ellipse.setAttribute("ry", getCurrentRy(currentLevel));
    ...
    ellipse.addEventListener(trigger, (e) => {
        if (currentLevel > 1) {
            ...
            e.target.addEventListener('animationend', function() {
                svg.value.removeChild(e.target);
            });
