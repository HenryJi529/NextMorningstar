...
const sendToAll = () => {
    ...
    stompClient.send("/api/ws/sendToAll", {}, JSON.stringify(chatMessage));
    console.log("广播消息发送成功");
}

const sendToOne = () => {
    let chatMessage: ChatMessage = {senderId: userStore.id, receiverId: receiverId.value, content: content.value};
    stompClient.send("/api/ws/sendToOne", {}, JSON.stringify(chatMessage));
...
<template>
    <div>
        ...
        <div>接收者：<input type="text" v-model="receiverId" /></div>
        <div>发送内容：<input type="text" v-model="content" /></div>
        <div>
            <button @click="sendToOne" class="btn">点对点</button>
            <button @click="sendToAll" class="btn">群发</button>
        </div>
