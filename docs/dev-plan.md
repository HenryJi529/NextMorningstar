...
- [x] 后端接口扩充(8/15–8/16,决策 45-48、51):`GET /dev/run` 列表 + `GET /dev/admin/stats` + `ProjectDetail`/`RunDetail` bo 扩充(adminName/projectName/prLink/deliveredIssueCount/actionAttemptBriefs/漏斗四值)+ `Run.triggerType`;原型 `frontend/prototype/dev.html` 定稿(决策 49)
- [ ] 三页实现(8/16,frontend-dashboard change 1.x/2.x/3.x):`/dev` 我的项目(吸顶子菜单/项目卡/状态机可视化+漏斗/历史表)、`/dev/admin` 系统管理(KPI/项目列表/正在运行/最近完成)、`/dev/about` 平台说明;状态机可视化组件两页共用
...
| 8/16 | 日 | 10h | 前端三页 + 录屏(死线 24:00) | ✅ Stats 已采纳/节约人天 + RunDetail 漏斗四值(决策 51);✅ 演示数据造数(StateMachineServiceTest 全链路,5 项目/6 run/三用户);✅ openspec 主 specs 全量同步;进行中:三页实现 → 录屏 |
