...
| 44 | **平台管理员权限模型**(8/15 定,已实现) | 复用既有权限框架(`application-perm.yml` 角色/权限点 + `@PreAuthorize`),不新建体系:角色 `dev_admin`(工坊管理员)+ 权限点 `dev:run:cancel`/`dev:project:disable`,账号分配走 yml `user-role` 段(dev/prod 同步)。管理员接口独立 `/dev/admin/**`,不改造 owner 接口:取消**任何** run、停用**任何**项目(只写 `enabled=false`,幂等,不级联取消进行中 run);管理员**无 enable 端点、无配置编辑权**(熔断权 vs 所有权分离,拉锯风险 MVP 接受,不加 `disabled_by` 字段)。**不留审计表**——降级为 `log.info` 留痕(操作人/目标/当时状态),`dev_admin_operation` 表结构与升级路径在 change design 留档。管理员角色无项目所有权语义但不主动禁止其作为普通用户创建项目(无用守卫);平台自跑项目(NextMorningstar nightly)归属专门服务账号,非管理员所有——owner/bot(HaibaraAi369)/管理员三层身份分离。见 admin-operations change |
| 45 | **run 列表与平台统计接口**(8/15 定,已实现) | `GET /dev/run`:`projectId`/`adminId` 可选过滤(AND 组合)、缺省全量、`create_time` 倒序,读公开;`dev_run` 无 admin_id 列,adminId 过滤先查归属项目再 `in`(**空集合必须短路**——MP `in` 空集生成非法 SQL)。`GET /dev/admin/stats` → `pojo/bo/Stats`:projectCount/executingRunCount/deliveredIssueCount/prTotal/prMerged;**字段用 Integer 不用 Long**——全局 `Long→ToStringSerializer` 配置下 Long 会序列化成 string,与前端 `number` 类型冲突。`deliveredIssueCount` 口径 = `status=SUCCEEDED` 的 run 下状态 VERIFIED/ACCEPTED/REJECTED 的 issue(失败/取消的 run 会 Restore 回滚,其 FIXED/VERIFIED 未交付不计;**REJECTED 计入**——修复数衡量 AI 能力,人工接受度由合并率 KPI 单独表达,两指标不纠缠)。统计读公开不加 `@PreAuthorize`("看数"非"操作",不为查看发明权限点) |
...
| 47 | **Detail bo 展示扩充模式**(8/15 定,已实现) | PO 不沾染展示字段,Service 出口统一返回 bo:`ProjectDetail extends Project`(补 `adminName`,取归属人 `username`——唯一登录名辨识度高,nickname 可选常为空不做 fallback 链)、`RunDetail extends Run`(补 `projectName`;另补 `prLink`——复用 `GiteaUtil.getPrLink` 拼 `backendOrigin/owner/repo/pulls/{prId}`,与 PR body 文件链接同 origin 口径,`project == null` 或 `prId == null` 时留空降级;`deliveredIssueCount`——交付口径同决策 45,仅 SUCCEEDED run 非 null,值为该 run 下 VERIFIED/ACCEPTED/REJECTED issue 总数,与 Stats 同词同口径避免词汇漂移);父子均 `@SuperBuilder`,`CopyUtil.copyNonNullProperties` 拷贝(source=PO 时 `getDeclaredFields` 边界无碍,field.set 对继承字段有效)。**单条 `toDetail` 为正典,批量即 `stream().map(this::toDetail)`**——MVP 不做批量查询优化(项目个位数,N+1 无感)。**关联对象为 null 时展示字段留空降级,不抛异常**(归属人被逻辑删除/项目已删而 run 日志留存均为真实场景;展示路径降级 ≠ 掩盖异常)。前端镜像同构:`types/dev.ts` 用 `interface XxxDetail extends Xxx` 继承(不直接改 PO 类型),axios 端点返回类型随迁;内部调用方(CronTask/AdminServiceImpl)零改动——Detail is-a PO 直接兼容 |
