...
| 32 | **FixAction 无 per-action 超时**(8/10 定) | 砍掉单 issue `--max-turns` + 整 run wall-clock + CancelTracker（原 claude-issue-fix design 决策 3）。复用全局 `CronTask.forceCleanStuckRuns`(120min + 每 5min 检测;8/23 由 `cancelTimeoutRuns` 改强制清理)兜底卡死。catch 只兜 `ProcessExecutionException`(+ FixAction 的 `JsonProcessingException`)，不补 `DataAccessException`(DB 失败概率极低，靠 CronTask) |
...
| 53 | **list 接口分页 + statuses 过滤**(8/17 定,已实现) | `GET /dev/run` 与 `GET /dev/project` 列表加必填 `pageNum`/`pageSize`(`@Positive` 照 blog 模块范式,缺参 Spring 400),返回 `PageResult`(MyBatis-Plus `selectPage`,`PaginationInnerInterceptor` 早已配置,正典在此);run 列表另加可选 `statuses` 过滤(`in`,前端逗号拼接、Spring 逗号分隔绑定 `List<Run.Status>`)——分页后"最近完成/历史任务"等终态视图不能再靠前端正过滤(进行中的 run 混入首页会造成缺行/空页),statuses 是正确性配套而非额外守卫;项目列表按 `create_time` 倒序(稳定分页需确定序)。前端三处表格(历史任务/平台项目列表/最近完成)同款 `PageSwitcher`(‹ 当前页/总页数 › 三元素,浅灰 hover 橙——8/22 由"仅当前页+悬浮显总页数"改为 a/b 常显,仅一页时组件自隐藏(隐藏判断收进组件,父级只传页码 props),置卡片标题行右侧——8/17 晚由表底挪入;轮询抽空末页自动回退一页)。非表格馈给用大页快照:项目胶囊 `pageSize=100`(子菜单需全量自己的项目)、平台当前任务 `statuses=RUNNING,CANCELING` + `pageSize=50`(覆盖并发槽+排队;8/18 复核维持 50:并发槽仅 4(max-concurrency),执行侧有界,50 = 4 执行 + 46 排队余量——无界部分仅夜间爆发 PENDING(≈启用项目数−4),即便极端截断,KPI 卡展示权威 executing/pending 计数不误导,故不动态化不提示溢出)、我的项目状态机 `pageSize=1` 取最新一条判定活跃(单项目同时至多一个活跃 run 且必为最新)。轮询从"一份全量列表多处用"拆为按用途的多路带参调用,同 3s 节奏。同日再简化:run 列表的 `adminId` 过滤(决策 45)无任何调用方,连同"先查归属项目再 `in`"段删除,最终签名 `projectId? + statuses? + pageNum + pageSize`。8/18 加必传 `sortDir` 枚举(`bo/SortDir` ASC/DESC,前端 `types/dev.ts` 同枚举对齐):平台当前任务 `sortDir=ASC` 按创建时间升序(先创建先跑/先排,贴合分发顺序——此前倒序 + 同秒并列按插入序恰巧呈升序,现显式固定),历史任务/最近完成/活跃探测 `sortDir=DESC` 维持倒序。排序方向必传无默认值,接口契约清晰,非法值经 Spring 枚举绑定直接 400。8/22 排序键按方向取各自语义(两列同向、互为兜底):ASC(平台当前任务)`createTime` 主序贴合分发顺序;DESC(历史/最近完成)`updateTime` 主序按完成时间排——按创建时间排时"先创建后完成"的 run 会沉底,与"最近完成"语义相悖;同秒并列由另一列保底确定序。 |
| 54 | **一轮 run 一个 PR、一晚一仓库一 PR(1 PR : N issue)**(8/24 定) | 同一 base 上并行开多个 PR 天然互相冲突——合并时只能合一个,其余全部冲突报废。因此修复串行:每个修复建立在前一个修好的代码基础上,逐 issue commit 线性叠加在同一 `fix/<runId>` 分支,一轮 run 只开一个 PR(`prId`/`prStatus` 挂 `dev_run` 而非 `dev_issue`);夜间定时一轮 = 一仓库一晚至多一个 PR。跨晚叠加靠下一轮 sync 对齐最新 origin(已合 PR 进新 base)+ 人工处置未合旧 PR。评审降负/验证复用是顺带收益,非决策依据 |
...
morningstar.app.dev:
  ...
  schedule:
    ...
    stuck-check-cron: "0 */5 * * * ?"     # 每 5min 卡死检测(8/23 由 timeout-cron 改名)
    ...
    stuck-threshold-minutes: 120         # 卡死判定阈值:N 分钟无状态流转即视为卡死,强制清理(8/23 由 run-timeout-minutes 改名)
