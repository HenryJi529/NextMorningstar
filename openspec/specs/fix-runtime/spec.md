...
为每次漏洞修复运行提供隔离、可驱动、可清理的标准化容器环境，以及该环境内代码的同步与还原能力。
...
- **AND** 若 `config` 命令失败 → 外层 `catch (ProcessExecutionException)` 兜底，同其他 git 操作
...
- **AND** **不裸抛**（裸抛 = attempt 停 RUNNING + run 卡中间态占并发槽等超时）
...
- **AND** issue 状态一并还原：本 run 的 `FIXED/VERIFIED → SELECTED`，清空 `commitSha`/`commitMessage`（FixAction 只修不回退，幂等重入由 RestoreAction 保证）
