...
- **WHEN** 存在进行中的 run
- **THEN** 每个 run 一张卡(项目名、runId 前 8 位可点击复制完整 runId、触发方式、迷你状态机、秒级 H:MM:SS 实时已耗时,与我的项目页"已耗时"同 `fmtElapsed` 口径);卡头右侧仅展示占槽信息(调度时段在顶栏胶囊,见"平台总览");进行中列表来自列表接口 `statuses=RUNNING,CANCELING` 首页大页快照(覆盖并发槽与排队,8/17 起)
