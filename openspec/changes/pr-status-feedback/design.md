...
新增 cron（建议每 5min，复用 schedule 配置体系 `pr-feedback-cron`）。只扫 `prId` 非空 + `prStatus=OPEN` 的 run（不限 `state`——CLEANED 后仍轮询）。PR 一旦 `MERGED`/`CLOSED` 即回写并停止轮询该 run。幂等：重复查同状态无副作用。
