...
import com.morningstar.infra.util.QrcodeUtil;
...
public class CommonServiceImpl implements CommonService {
    ...
    public ImageCaptchaResponseVo getImageCaptcha() {
        ...
        // 生成sessionId
        String sessionId = Long.toHexString(snowflake.nextId());
        // 将sessionId和验证码保存在redis下，并设置缓存中数据存活时间五分钟
        redisTemplate.opsForValue().set(RedisConstant.AUTH_CHECK + RedisConstant.KEY_SEPARATOR + sessionId, captcha.getCode(), 5, TimeUnit.MINUTES);
    ...
    public void getEmailCaptcha(String email) {
        ...
        redisTemplate.opsForValue().set(RedisConstant.AUTH_CHECK + RedisConstant.KEY_SEPARATOR + email, code, 5, TimeUnit.MINUTES);
