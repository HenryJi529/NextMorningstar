...
import com.morningstar.infra.properties.SiteProperties;
...
import com.morningstar.infra.util.GithubUtil;
import com.morningstar.infra.util.QiniuUtil;
import com.morningstar.infra.util.RandomUtil;
import com.morningstar.system.converter.UserConverter;
import com.morningstar.system.dao.mapper.RoleMapper;
import com.morningstar.system.dao.mapper.UserMapper;
import com.morningstar.system.dao.mapper.UserRoleMapper;
import com.morningstar.system.pojo.bo.*;
...
import com.morningstar.system.properties.JwtProperties;
import com.morningstar.system.properties.OAuth2Properties;
...
public class UserServiceImpl implements UserService {
    ...
    private void preCheckImageCaptcha(ImageCaptchaProtectedRequestVo vo) {
        String redisCode = (String) redisTemplate.opsForValue().get(RedisConstant.AUTH_CHECK + RedisConstant.KEY_SEPARATOR + vo.getSessionId());
        checkCode(redisCode, vo.getCode());
        redisTemplate.delete(RedisConstant.AUTH_CHECK + RedisConstant.KEY_SEPARATOR + vo.getSessionId());
    }
    ...
    private UserPublicInfo getPublicInfo(User dbUser) {
        UserPublicInfo userPublicInfo = userConverter.userToUserPublicInfo(dbUser);
        ...
        userPublicInfo.setIsOnline(redisTemplate.opsForValue().get(RedisConstant.AUTH_HEARTBEAT + RedisConstant.KEY_SEPARATOR + userPublicInfo.getId()) != null);
    ...
    private UserEditableInfo getEditableInfo(UUID userId) {
        ...
        return userConverter.userToUserEditableInfo(dbUser);
    }
    ...
    private UserPrivateInfo getPrivateInfo(LoginUser loginUser) {
        UserPrivateInfo userPrivateInfo = userConverter.userToUserPrivateInfo(loginUser.getUser());
    ...
    public RegisterResponseVo register(RegisterRequestVo vo) {
        ...
        // 插入数据库
        User dbUser = userConverter.registerRequestVoToUser(vo);
    ...
    public LogoutResponseVo logout() {
        ...
        redisTemplate.delete(RedisConstant.AUTH_LOGIN + RedisConstant.KEY_SEPARATOR + userid);
    ...
    public UserEditableInfo updateEmail(UpdateEmailRequestVo vo) {
        String redisCode = (String) redisTemplate.opsForValue().get(RedisConstant.AUTH_CHECK + RedisConstant.KEY_SEPARATOR + vo.getEmail());
        checkCode(redisCode, vo.getCode());
        redisTemplate.delete(RedisConstant.AUTH_CHECK + RedisConstant.KEY_SEPARATOR + vo.getEmail());
    ...
    public String getRandomNickname() {
        return systemFakeUtil.getRandomNickname();
    }

    @Override
    public String getRandomNickname(double latitude, double longitude) {
        return systemFakeUtil.getRandomNickname(latitude, longitude);
    }
    ...
    public void updateIsLocked(UUID id, Boolean isLocked) {
        ...
        if (isLocked) {
            redisTemplate.delete(RedisConstant.AUTH_LOGIN + RedisConstant.KEY_SEPARATOR + id);
        }
    }
    ...
    private void sendRecoverEmail(UUID id, String email) {
        ...
        redisTemplate.opsForValue().set(RedisConstant.AUTH_RECOVER + RedisConstant.KEY_SEPARATOR + id, token, 10, TimeUnit.MINUTES);
    ...
    public CompleteRecoveryResponseVo completeRecovery(CompleteRecoveryRequestVo vo) {
        String redisToken = (String) redisTemplate.opsForValue().get(RedisConstant.AUTH_RECOVER + RedisConstant.KEY_SEPARATOR + vo.getId());
        ...
        redisTemplate.delete(RedisConstant.AUTH_RECOVER + RedisConstant.KEY_SEPARATOR + vo.getId());
    ...
    public OAuthResponseVo githubOAuthCallback(String code) {
        ...
        if (dbUser == null) {
            ...
            dbUser = User.builder()
                    ...
                    .avatar(avatar)
                    .nickname(systemFakeUtil.getRandomNickname())
                    .build();
            userMapper.insert(dbUser);
        }
