...
public class UserServiceImpl implements UserService {
    ...
    private void updateLoginUserInRedis() {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        LoginUser oldLoginUser = (LoginUser) authentication.getPrincipal();
        String username = userMapper.selectById(oldLoginUser.getUser().getId()).getUsername(); // 可以处理用户名更改的情况
        LoginUser newLoginUser = (LoginUser) userDetailsService.loadUserByUsername(username);
        redisTemplate.opsForValue().set(RedisConstant.LOGIN_PREFIX + newLoginUser.getUser().getId(), newLoginUser);
    }

    private void updateLoginUserInRedis(UUID id){
