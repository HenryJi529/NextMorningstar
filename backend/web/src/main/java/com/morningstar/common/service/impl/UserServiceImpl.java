...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public R<Object> githubOAuthCallback(String code) {
        ...
        JsonNode githubUser = githubUtil.getUser(githubToken);
        String username = "github_" + githubUser.get("login").asText();

        User dbUser = userMapper.selectByUsername(username);
        if(dbUser == null){
            /* 如果用户不存在就注册 */
            String email = githubUser.get("email").asText();
            // 图片保存到七牛云
            String avatar = new Date().getTime() + ".jpeg";
            byte[] data = Objects.requireNonNull(restTemplate.getForObject(githubUser.get("avatar_url").asText(), byte[].class));
        ...
        // 组装注册成功数据
        OAuthResponseVo respVo = new OAuthResponseVo(loginUser.getUser().getId(), loginUser.getUsername(), jwtUtil.create(loginUser.getUser().getId(), loginUser.getUsername(), ""));
        return R.ok(respVo);
    }
}
