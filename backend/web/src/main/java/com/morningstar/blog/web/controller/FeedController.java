...
public class FeedController {
    ...
    private String getBlogRootUrl(){
        return rootUrl + blogProperties.getRootPath();
    }
    ...
    public View getRss() {
        return new AbstractRssFeedView() {
            @Override
            protected void buildFeedMetadata(@NotNull Map<String, Object> model,
                                             @NotNull Channel feed, @NotNull HttpServletRequest request) {
            ...
            @NotNull
            @Override
            protected List<Item> buildFeedItems(@NotNull Map<String, Object> model, @NotNull HttpServletRequest request, @NotNull HttpServletResponse response) {
                ...
                return articleDetailList.stream().map(articleDetail -> {
                    ...
                    item.setPubDate(articleDetail.getArticle().getUpdateTime());
    ...
    @GetMapping("/atom.xml")
    public View getAtom(){
        String blogRootUrl = rootUrl + blogProperties.getRootPath();

        return new AbstractAtomFeedView() {
            @Override
            protected void buildFeedMetadata(@NotNull Map<String, Object> model, @NotNull Feed feed, @NotNull HttpServletRequest request) {
                feed.setTitle(blogProperties.getAtomTitle());

                Link link = new Link();
                link.setHref(blogRootUrl);
            ...
            @NotNull
            @Override
            protected List<Entry> buildFeedEntries(@NotNull Map<String, Object> model, @NotNull HttpServletRequest request, @NotNull HttpServletResponse response) {
                List<ArticleDetail> articleDetailList = getArticleDetailList();
                return articleDetailList.stream().map(articleDetail -> {
                    Entry entry = new Entry();

                    entry.setTitle(getArticleTitle(articleDetail));
                    entry.setUpdated(articleDetail.getArticle().getUpdateTime());
                    entry.setCreated(articleDetail.getArticle().getCreateTime());
