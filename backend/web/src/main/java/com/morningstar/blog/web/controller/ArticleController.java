...
public class ArticleController {
    ...
    public R<ArticleDetail> getArticleDetailByArticleId(@PathVariable Long id) {
        articleService.recordArticleView(id);
        return R.ok(articleService.getArticleDetailByArticleId(id));
    }

    @Operation(summary = "获取摘要(根据id)")
    @GetMapping("/summary/{id}")
    public R<String> getArticleSummaryByArticleId(@PathVariable Long id) {
        return R.ok(articleService.getArticleSummaryByArticleId(id));
    }

    @Operation(summary = "刷新摘要(全部)")
    @PreAuthorize("hasAuthority('blog:article:manage')")
    @PostMapping("/summary/refresh")
    public R<Object> refreshArticleSummary() {
        articleService.refreshArticleSummary();
        return R.ok();
    }
    ...
    @Operation(summary = "刷新文章检索库")
    @PreAuthorize("hasAuthority('blog:article:manage')")
    @PostMapping("/search/refresh")
    public R<Object> refreshArticleSearch() {
        articleService.refreshArticleSearch();
        return R.ok();
    }
    ...
    @PreAuthorize("hasAuthority('blog:article:manage')")
    public R<Object> updateArticle(@PathVariable Long id, @Valid @RequestBody UpdateArticleRequestVo vo) {
    ...
    @PreAuthorize("hasAuthority('blog:article:manage')")
    public R<Object> deleteArticle(@PathVariable Long id) {
