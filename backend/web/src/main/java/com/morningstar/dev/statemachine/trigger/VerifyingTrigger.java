...
import com.morningstar.dev.statemachine.action.VerifyAction;
...
@Slf4j
public class VerifyingTrigger implements Trigger {
    private final VerifyAction verifyAction;

    @Override
    @EventListener
    public void onStateChanged(StateChangedEvent event) {
        if (event.getToState() != State.VERIFYING) {
            return;
        }
        verifyAction.execute(event.getRunId());
    }
}
