...
import com.morningstar.dev.statemachine.action.StartAction;
...
@Slf4j
public class StartingTrigger implements Trigger {
    private final StartAction startAction;

    @Override
    @EventListener
    public void onStateChanged(StateChangedEvent event) {
        if (event.getToState() != State.STARTING) {
            return;
        }
        startAction.execute(event.getRunId());
    }
}
