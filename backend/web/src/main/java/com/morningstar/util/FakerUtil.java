...
public class FakerUtil {
    ...
    public Comment fakeBlogComment(){
        ...
        if(parentComment != null) {
            long timeDiff = new Date().getTime() - parentComment.getCreateTime().getTime();
            dateCreated = new Date(parentComment.getCreateTime().getTime() + random.nextLong(timeDiff));
        }else{
            long timeDiff = new Date().getTime() - article.getCreateTime().getTime();
            dateCreated = new Date(article.getCreateTime().getTime() + random.nextLong(timeDiff));
        }
        long timeDiff = new Date().getTime() - dateCreated.getTime();
        Date dateUpdated = new Date(dateCreated.getTime() + random.nextLong(timeDiff));
    ...
    public Record fakeKillRecord(long earliestCreateMilliseconds){
        ...
        GameLog gameLog = new GameLog(mode, users.stream().map(KillUser::new).collect(Collectors.toList()), winners.stream().map(KillUser::new).collect(Collectors.toList()));
        ...
        earliestCreateMilliseconds = Math.max(lastUserCreateTime.getTime(), earliestCreateMilliseconds);
