...
public class ClassUtil {
    ...
    public static <T> Map<Class<? extends T>, T> loadInstancesByPackageName(Class<T> parentClass, String packageName) {
        List<Class<?>> classes = loadClassesByPackageName(packageName);
        Map<Class<? extends T>, T> instances = new HashMap<>();
        for (Class<?> clazz : classes) {
            // 检查是否是指定父类的子类
            if (parentClass.isAssignableFrom(clazz)) {
                // 安全向下转型 asSubclass 替代裸强转
                Class<? extends T> subClazz = clazz.asSubclass(parentClass);
                // 创建类的实例【如果没有空参构造会报错】
                T instance;
                try {
                    instance = subClazz.getDeclaredConstructor().newInstance();
                } catch (InstantiationException | IllegalAccessException | InvocationTargetException |
                         NoSuchMethodException e) {
                    throw new RuntimeException("无法通过无参构造创建类的实例");
                }
                // 将实例添加到Map中
                instances.put(subClazz, instance);
            }
        }
        return instances;
    }
