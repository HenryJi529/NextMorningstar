package com.morningstar.kill.domain.hero.hero;

import com.morningstar.kill.domain.hero.Faction;
import com.morningstar.kill.domain.hero.Hero;
import com.morningstar.kill.domain.hero.Sex;
import com.morningstar.kill.domain.skill.SkillPool;
import com.morningstar.kill.domain.skill.skill.挑衅;
import com.morningstar.kill.domain.skill.skill.志继;

import java.util.HashSet;
import java.util.List;


public class 姜维 extends Hero {
    public 姜维() {
        super(
                4,
                Sex.MALE,
                Faction.SHU,
                "权力欲重的蜀汉后期顶梁柱",
                new HashSet<>(List.of(SkillPool.getSkillByClass(挑衅.class), SkillPool.getSkillByClass(志继.class))),
                false
        );
    }
}
