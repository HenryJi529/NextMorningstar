...
ENV_DICT["CLOUD_IP"] = get_ip_address(Env.CLOUD_DOMAIN)
...
class MorningstarConnection(Connection):
    ...
    def run(self, command: str, **kwargs):
        return super().run(command, env={**ENV_DICT, **kwargs.pop('env', {})}, **kwargs)
...
class Commands:
    ...
    def buildProd():
        colored_print("前端打包...")
        runcmd(f'cd {LOCAL_BASE_DIR.parent}/frontend && npm run build')
        colored_print("后端打包...")
        runcmd(f'cd {LOCAL_BASE_DIR.parent}/backend && mvn clean package -Pprod -DskipTests')
